...
# document 分类  有可能维度（用来文本分类的文本特征）太大
# pca 用来降维 冗余信息太多，导致分类效果不好 --- 用pca来包含只对于文本分类有用的信息
# 先用所有的特征来进行一个无监督的训练，得到一个模型，将原来的模型降维到想要的维度
# 用pca降维后会告诉你 还包含了多少信息， 最终降到一个维度（尽量小）同时信息保留的也不错


class Dataset:
    """Dataset for classification."""

    def __init__(self, data: npt.NDArray[np.float_], labels: npt.NDArray[np.int_]):
        """Initialize."""
        self.data = data
        self.labels = labels
    ...
    def percent_correct(self):
        ...
        This happens to also work if the data are estimated labels themselves.
        """
        max_correct = 0.0
        for threshold in self.data.ravel():
            labels_below = self.labels[self.data.ravel() < threshold]
            labels_above = self.labels[self.data.ravel() >= threshold]
            correct = (sum(labels_below == 0) + sum(labels_above == 1)) / self.data.size
...
class FLD:
    ...
    def train(self, dataset: Dataset):
        ...
        if Sigma_1.size == 1:
            self.w = (mu_1 - mu_0) / (Sigma_1 + Sigma_0)
        else:
...
def generate_data(
    ...
    # The following converts these into one-hot vectors instead.
...
class PCA:
    ...
    def train(self, dataset: Dataset):
        ...
        self.w = evec[:, np.argmax(eval)]
        return self

    def apply(self, dataset: Dataset):
        """Apply."""
        return Dataset(dataset.data @ np.expand_dims(self.w, axis=1), dataset.labels)
...
def plot_correct_vs_dims():
    ...
    for D in num_dims:
        ...
        for _ in range(replicates):
            ...
            # correctness_cod_D.append(guess_dataset_test.percent_correct())

            # pca = PCA().train(dataset_train)
            # knn = KNN(5).train(pca.apply(dataset_train))
            # guess_dataset_test = knn.apply(pca.apply(dataset_test))
            # correctness_pca_D.append(guess_dataset_test.percent_correct())

            fld = FLD().train(dataset_train)
            proj_dataset_test = fld.apply(dataset_test)
            correctness_cod_D.append(proj_dataset_test.percent_correct())

            pca = PCA().train(dataset_train)
            fld = FLD().train(pca.apply(dataset_train))
            proj_dataset_test = fld.apply(pca.apply(dataset_test))
            correctness_pca_D.append(proj_dataset_test.percent_correct())

        correctness_cod.append(np.mean(correctness_cod_D))
        correctness_pca.append(np.mean(correctness_pca_D))
    plt.plot(num_dims, correctness_cod)
    plt.plot(num_dims, correctness_pca)
...
def main():
    ...
    dataset = generate_data(num_dims, num_classes, num_samples, R)
    dataset.plot()
    plt.show()

    pca_dataset = PCA().train(dataset).apply(dataset)
    pca_dataset.plot()
    plt.show()
